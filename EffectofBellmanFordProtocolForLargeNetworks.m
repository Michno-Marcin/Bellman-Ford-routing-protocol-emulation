...
% Assigning random weights from the range to all network channels: 1-100
weights = round(99*rand(1,AmountOfLinks))+1;
% Generating random channels in the network (identifying sources and outlets)
source_nodes=round((AmountOfNodes-1)*rand(1,AmountOfLinks))+1;
outlets_nodes=round((AmountOfNodes-1)*rand(1,AmountOfLinks))+1;

% Generate node names by creating a "No.?" where "?" means the next node number
names=string;
 for x=AmountOfNodes:-1:1
     names(x)=convertCharsToStrings(['No.',num2str(x)]);
 end
%names % <- optional checking channel names

%% Visualization of the created network graph

 NetworkGraph = graph(outlets_nodes,source_nodes,weights,names);
% Removal of multiple channels, if any, replacing them with single channels 
if ismultigraph(NetworkGraph)
    NetworkGraph = simplify(NetworkGraph,'sum'); 
    % The weight of the edge that is left is equal to the sum of the replaced edge weights
end
% Removal self-loops, if any
NetworkGraph = simplify(NetworkGraph,'omitselfloops'); 
...
GPlotGraph = plot(NetworkGraph,'XData',x,'YData',y,'EdgeLabel',...
    NetworkGraph.Edges.Weight,'LineWidth', ...
    2*NetworkGraph.Edges.Weight/max(NetworkGraph.Edges.Weight)); % Graph visualization
